...
class MultiScaleRoIAlign(nn.Module):
    ...
    def forward(
        ...
        if torchvision._is_tracing():
            stacked_features = torch.cat(tracing_results, dim=0)
            stacked_indices = torch.cat(indices, dim=0)
            return stacked_features[stacked_indices.argsort()]
